...
function init() {
    ...
    let stars = document.querySelectorAll("span#stars img");
...
function lightStars(evt) {
    let starNumber = evt.target.alt;
    ...
    // ranging from the value of the starNumber variable to less than 5. Unlight every star in this col-
    ...
    // f. When the mouse pointer moves off a star image, the lit stars should be unlit. Add an event
    // listener to the target of the event object that runs the turnOffStars() function in response to the 
    // mouseleave event.
    evt.target.addEventListener("mouseleave", turnOffStars);
...
function updateCount() {
    ...
    let wordCountBox = document.getElementById("wordCount");
